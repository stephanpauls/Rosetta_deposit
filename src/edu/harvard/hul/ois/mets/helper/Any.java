    ...
    extends MetsElement
{
    ...
    public static Any reader (MetsReader r,XmlData xml)
	throws MetsException
    {
	Any any = new Any ();
	any.read (r,xml);
    ...
    public void read (MetsReader r,XmlData xml)
    		throws MetsException
    	    {
    		Token token = r.getStart ();
    		_qName     = token.getQName ();
    		...
    		_attrs     = token.getAttributes ();
    	        ...
    	        if (token.getType().equals(Type.CDATA)) {
    	        	xml.setcData(token.getValue());
    	        	return;
    	        }
    		while ((token = r.atContent ()) != null) {
    		    Type type = token.getType ();
